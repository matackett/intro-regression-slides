...
The data set contains the **heart weight** (.vocab[`Hwt`]) and **body weight** (.vocab[`Bwt`]) for 144 domestic cats.
...
We want to fit a model that uses a cat's body weight to predict how much its heart weighs.
...
We can use the regression model to predict 

--

1. The .vocab[<u>mean</u>] response when the predictor variable is equal to a value $x_0$

--

2. The response for an .vocab[<u>individual</u>] observation with a value of the predictor equal to $x_0$

--

The predicted value will be the same in both cases, but the uncertainty around the prediction will differ. 
...
$$\hat{y} \pm t^* \times \color{purple}{\mathbf{SE}_{\hat{\boldsymbol{\mu}}}}$$
]

.eq[
**Prediction interval for an individual observation**
$$\hat{y} \pm t^* \times \color{purple}{\mathbf{SE_{\hat{y}}}}$$
]
...
ggplot(data = cats, aes(x = Bwt, y = Hwt)) +
  ...
  labs(x = "Body Weight (kg)",
       y = "Heart Weight (g)") +
  annotate("text", x = 8.2, y = 17, label = "Andy", color = "red") +
  theme_bw()
```
...
- Predicted the repsonse given a value of the predictor variable
