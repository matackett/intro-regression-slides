...
.eq[
$$\begin{aligned}Y &= \color{purple}{f(X)} + \epsilon \\[8pt]
...
where the errors are independent and normally distribution, $\epsilon \sim N(0, \sigma_\epsilon)$

---

class: middle

$$Y|X \sim N(\beta_0 + \beta_1 X, \sigma_\epsilon)$$
...
geom_vline(xintercept=breaks, lty=2, color = "grey") +
  labs(x = "x", 
       y = "y") +
  theme_classic() + 
  annotate("text", x = 10, y = 600, label = latex2exp::TeX("$\\mu_Y = \\beta_0 + \\beta_1X$"), color = "steelblue", size = 8) +
  theme(axis.title = element_text(size = 16),
        axis.ticks = element_blank(), 
       axis.text.x = element_blank(), 
       axis.text.y = element_blank()
...
geom_vline(xintercept=breaks, lty=2, color = "grey") +
  ...
  theme_classic() + 
  annotate("text", x = 10, y = 600, label = latex2exp::TeX("$\\mu_Y = \\beta_0 + \\beta_1X$"), color = "steelblue", size = 8) +
  annotate("text", x = 20, y = 400, label = latex2exp::TeX("$\\sigma_\\epsilon$"), color = "red", size = 8) +
  theme(axis.title = element_text(size = 16),
        axis.ticks = element_blank(), 
        axis.text.x = element_blank(), 
       axis.text.y = element_blank()
...
Once we fit the model, we can use the residuals to to calculate the vocab[regression standard error, $\hat{\sigma}_\epsilon$]
